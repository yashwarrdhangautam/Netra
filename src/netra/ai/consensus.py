...
def calculate_consensus(analyses: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "bounty_hunter_applied": bool(analyses.get("bounty_hunter")),
    }


def skeptic_veto_applies(analyses: dict[str, Any], threshold: float = 0.80) -> bool:
    """Return true when Skeptic should block bounty-specific prioritization."""
    skeptic = analyses.get("skeptic", {})
    ...
    return verdict == "false_positive" or (
        verdict == "likely_false_positive" and score >= threshold
    )
