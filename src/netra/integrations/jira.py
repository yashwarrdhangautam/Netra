...
class JiraClient:
    ...
    def __init__(
        ...
        if base_url:
            ...
            try:
                ...
                msg = f"Jira base URL points to internal/private address: {base_url}"
        ...
        # Use proper URL parsing to check cloud status — avoid substring matching (CodeQL CWE-20)
        if self.base_url:
            from urllib.parse import urlparse
            parsed_host = urlparse(self.base_url).hostname or ""
            self.is_cloud = parsed_host.endswith(".atlassian.net") or parsed_host == "atlassian.net"
        else:
            self.is_cloud = False
    ...
    def _build_description(self, finding: dict[str, Any], scan: dict[str, Any]) -> str:
        ...
        lines.extend([
            "h3. Metadata",
            "",
            f"_Generated by NETRA Security Platform on {datetime.now(UTC).isoformat()}_",
        ])
